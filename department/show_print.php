...
if(isset($_POST['id']))
	$sql_order = "SELECT *, employees.rate as rt, employees.eid AS empid FROM employees LEFT JOIN position ON position.id=employees.position_id LEFT JOIN schedules ON schedules.id=employees.schedule_id WHERE employees.eid='".$_POST['id']."'";
         ...
         $sql1 = "SELECT company from admin WHERE id='".$_SESSION['admin_id']."'";
        $result1 = $conn->query($sql1);
        $row1=mysqli_fetch_assoc($result1);
 //$qrcodetxt = 'ID:'.$id.',Admin ID:'.$admin_id.', NAME: '.$full_name.', Position: '.$position.', Company :'.$row1['company'].', Department :'.$_SESSION["department_name"].'';
    ...
    <div style="clear:both;">
      <table class="table" cellspacing="0" border="0">
        ...
        <tbody>
            ...
            <td style="text-align:right; font-size: 10px;"><?php echo $row1['company']; ?></td>
          </tr>
          <tr>
          ...
          <tr>
            <td style="text-align:center;font-size: 10px;"><img src="<?php echo (!empty($row['image']))? '../admin/uploaded/employee/'.$row['image']:'../admin/uploaded/profile.jpg'; ?>" style="width: 70px;"></td>
