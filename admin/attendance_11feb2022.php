							   ...
							   <div class="form-validation">
								   <form class="form-valide" action="attendance.php" method="POST">
									   ...
									   <div class="form-group row">
										   ...
										   <div class="col-lg-6">
                                           <select class="form-control" name="emp_name" id="emp_name"></select>
										   </div>
									   </div>
									   <div class="form-group row">
									   ...
									   else if(isset($_POST['show_record']))
									   {
										...
										$dpt=$_POST['dept'];
										  ...
										  $from = date('Y-m-d', strtotime($ex[0]));
										   ...
										   if($_POST['emp_name'])
										   {
												   $sql = "SELECT *, employees.employee_id AS empid, attendance.id AS attid,attendance.status as attend_status FROM attendance LEFT JOIN employees ON employees.eid=attendance.employee_id WHERE attendance.date BETWEEN '$from' AND '$to' AND employees.admin_id='".$_SESSION['id']."' AND employees.eid='$employid' ORDER BY attendance.date DESC, attendance.time_in DESC LIMIT $start_from, $limit";
										   }
										   else if($dpt!=='all'){
												$sql = "SELECT *, employees.employee_id AS empid, attendance.id AS attid,attendance.status as attend_status FROM attendance LEFT JOIN employees ON employees.eid=attendance.employee_id WHERE attendance.date BETWEEN '$from' AND '$to' AND employees.admin_id='".$_SESSION['id']."' AND employees.department_id='".$_POST['dept']."' ORDER BY attendance.date DESC, attendance.time_in DESC LIMIT $start_from, $limit";
											 
											}
										   else
										   if($_POST['dept']=='all')
										   {
											   $sql = "SELECT *, employees.employee_id AS empid, attendance.id AS attid,attendance.status as attend_status FROM attendance LEFT JOIN employees ON employees.eid=attendance.employee_id WHERE attendance.date BETWEEN '$from' AND '$to' AND employees.admin_id='".$_SESSION['id']."' ORDER BY attendance.date DESC, attendance.time_in DESC LIMIT $start_from, $limit";
										}
									   ...
									   ?>
										   ...
										   <tr>
											   ...
											   <td><?php echo $timeout; ?></td>
												<td><?php echo $row['time_out_remark']; ?></td>
