...
function Header()
{
    ...
    $this->Cell(150);
    // Title
    $this->Cell(40,10,'INVOICE',1,0,'C');
    // Line break
    $this->Ln(15);

    include "connect.php";
    $sql_get_inv = mysqli_query($conn,"SELECT * FROM inv_payment WHERE id='".$_GET['inv_id']."'");
    $row_inv = mysqli_fetch_array($sql_get_inv);

    $sql_get_qut = mysqli_query($conn,"SELECT * FROM quotation WHERE id='".$row_inv['qut_id']."'");
    $row_qut = mysqli_fetch_array($sql_get_qut);
     
    	$this->SetFont('Arial','B',12);
    	$this->Cell(150);
    	$this->Cell(40,0,$row_inv['inv_id'],0,0,'C');
    	$this->Ln(15);
        $this->Cell(150);
        $this->Cell(40,0,$row_inv['inv_status'],0,0,'C');
        $this->Ln(15);
