...
");
    ...
    $currentEmployees = (int)$empRow['total'];

    // active subscription from admin table
...
");
    ...
    // defaults
    $maxEmployees  = $subRow ? (int)$subRow['max_employees'] : 0;
    $packageName  = $subRow['package_name'] ?? 'No Active Package';

    // check limit (0 = unlimited)
    $limitReached = false;
    if ($subRow && $maxEmployees != 0 && $currentEmployees >= $maxEmployees) {
        $limitReached = true;
    }
    if ($subRow && $maxEmployees != 0 && $currentEmployees >= $maxEmployees) {
        echo "<script>
            alert('Your subscription package ($packageName) allows only $maxEmployees employees and this limit is completed. Please upgrade your subscription.');
            window.location='employeelist.php';
        </script>";
        exit;
    }
    if (isset($_POST['save']) && !$limitReached) {
        ...
        VALUES ('" . $_SESSION['id'] . "','$employee_id','$pass', '$firstname', '$lastname', '$email', '$address', '$birthdate', '$contact', '$gender', '$position', '$rate', '$schedule', '$leave','$leave' ,'$leavediff','$sick_allot_leave','$sick_allot_leave', '$image', NOW(),'" . $_SESSION["expiry_date"] . "','$department_id','$pin')";
